...
public class QueryRuleSqlBuilder {
    ...
    private List<Sort> sorts; //保存排序规则列表
     ...
     * @return
     */
    protected String removeSorts(String sql) {
     ...
     * @param rule 查询规则
     */
    private void processSort(QueryRule.Rule rule) {
        switch (rule.getType()) {
            case QueryRule.ASC_ORDER:
                // propertyName非空
                if (!StringUtils.isEmpty(rule.getPropertyName())) {
                    sorts.add(Sort.asc(rule.getPropertyName()));
                }
                break;
            case QueryRule.DESC_ORDER:
                // propertyName非空
                if (!StringUtils.isEmpty(rule.getPropertyName())) {
                    sorts.add(Sort.desc(rule.getPropertyName()));
                }
                break;
            default:
                break;
    ...
    private void appendWhereSql() {
        ...
        this.whereSql = removeSelect(removeSorts(whereSql.toString()));
    }

    /**
     * 拼装排序语句
     */
    private void appendSortSql() {
        StringBuffer sortSql = new StringBuffer();
        for (int i = 0; i < sorts.size(); i++) {
            if (i > 0 && i < sorts.size()) {
                sortSql.append(",");
            }
            sortSql.append(sorts.get(i).toString());
        }
        this.sortSql = removeSelect(removeSorts(sortSql.toString()));
    }
